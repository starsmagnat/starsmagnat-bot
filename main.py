        ...
        try:
            ...
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS jackpot (
                    ...
                    amount DECIMAL(15, 2) DEFAULT 20.0,
                    CONSTRAINT single_row CHECK (id = 1)
                )
            ''')
            # Инициализация джекпота если его нет
            await conn.execute('''
...
async def get_jackpot_amount():
    async with db_pool.acquire() as conn:
        amount = await conn.fetchval('SELECT amount FROM jackpot WHERE id = 1')
        return float(amount) if amount is not None else 5.0

async def add_to_jackpot(amount: float):
    async with db_pool.acquire() as conn:
        await conn.execute('UPDATE jackpot SET amount = amount + $1 WHERE id = 1', Decimal(str(amount)))

async def reset_jackpot():
    async with db_pool.acquire() as conn:
        await conn.execute('UPDATE jackpot SET amount = 20.0 WHERE id = 1')

async def roll_fortune_wheel(user_id: int):
    # Призы и вероятности (всего 2000 ед для точности 0.05%)
    # 0.5 звезд -- 35% -> 700
    # 1 звезда -- 25% -> 500
    # 1.5 звезды -- 15% -> 300
    # 2 звезды -- 10% -> 200
    ...
    # 10 звезд -- 0.6% -> 12
    # 20 звезд -- 0.3% -> 6
    # Джекпот -- 0.05% -> 1 (было 0.1% -> 2)
    # Повышаем 0.5 звезд на 0.05% -> +1 -> 701
    prizes = [
        (0.5, 701), (1.0, 500), (1.5, 300), (2.0, 200), 
        (2.5, 160), (3.0, 80), (5.0, 40), (10.0, 12), 
        (20.0, 6), ("JACKPOT", 1)
    ]
    
    total_weight = sum(p[1] for p in prizes)
    rand_val = random.randint(1, total_weight)
    
    current_weight = 0
    selected_prize = prizes[0][0]
    ...
    # Каждый прокрут добавляет 0.02 к джекпоту
    await add_to_jackpot(0.02)
    
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Проверяем последний бонус
            row = await conn.fetchrow('SELECT last_bonus, username, name FROM users WHERE user_id = $1 FOR UPDATE', user_id)
            ...
            # Проверяем реферальную систему: награждаем пригласителя, если это первый прокрут
            if row['last_bonus'] == 0:
                async with db_pool.acquire() as conn_ref:
                    ref_id = await conn_ref.fetchval('SELECT referrer_id FROM referral_connections WHERE user_id = $1', user_id)
                    if ref_id:
                        print(f"[REFERRAL] First spin! Rewarding referrer {ref_id} for user {user_id}")
                        await conn_ref.execute('UPDATE users SET balance = balance + 2, refs = refs + 1 WHERE user_id = $1', ref_id)
                        await log_action(ref_id, 'referral_reward', 2, {'referred_user': user_id})
                        try:
                            await bot.send_message(ref_id, f"🎁 <b>Ваш реферал открыл свой первый кейс!</b>\n\nВам начислено 2 ⭐️", parse_mode='HTML')
                        except: pass

            reward_amount = 0.0
            is_jackpot = False
            
            if selected_prize == "JACKPOT":
                is_jackpot = True
                reward_amount = await get_jackpot_amount()
                await reset_jackpot()
                ...
                try:
                    admin_text = (
                        f"🎰 <b>ДЖЕКПОТ ВЫБИТ!</b>\n\n"
                        f"👤 Пользователь: {row['name']} (@{row['username'] if row['username'] else 'нет'})\n"
                        f"🆔 ID: <code>{user_id}</code>\n"
                        f"💰 Сумма джекпота: {reward_amount:.2f} ⭐️"
                    )
                    await bot.send_message(ADMIN_ID, admin_text, parse_mode="HTML")
                except Exception as e:
                    print(f"Error sending jackpot notification: {e}")
            else:
                reward_amount = float(selected_prize)
            
            await conn.execute(
                'UPDATE users SET balance = balance + $1, last_bonus = $2 WHERE user_id = $3',
                Decimal(str(reward_amount)), now, user_id
            )
...
async def admin_jackpot_cmd(message: types.Message):
    amount = await get_jackpot_amount()
    await message.answer(f"💰 Текущая сумма джекпота: {amount:.2f} ⭐️")
...
async def cleanup_old_records():
    async with db_pool.acquire() as conn:
        ...
        print(f"[CLEANUP] Deleted old records")

async def get_required_channels():
    async with db_pool.acquire() as conn:
        rows = await conn.fetch('SELECT channel_id, url, name FROM required_channels')
        return [dict(row) for row in rows]

async def add_required_channel(channel_id: int, url: str, name: str):
    async with db_pool.acquire() as conn:
        await conn.execute(
            '''INSERT INTO required_channels (channel_id, url, name) 
               VALUES ($1, $2, $3)
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            '''INSERT INTO action_logs (user_id, action_type, amount, details, created_at) 
               VALUES ($1, $2, $3, $4, NOW())
               RETURNING id''',
            user_id, action_type, Decimal(str(amount)), json.dumps(details) if details else None
        )
...
async def check_subscription(user_id: int) -> bool:
    ...
    for channel in channels:
        try:
            ...
            # Если бот не админ или канал не найден, считаем что ок, чтобы не блокировать юзера
            continue
    return True
...
async def send_subscription_message(chat_id: int):
    ...
    markup = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text=ch['name'], url=ch['url'])] for ch in channels
    ])
    markup.inline_keyboard.append([types.InlineKeyboardButton(text="✅ Проверить подписку", callback_data='check_sub')])
...
async def handle_check_sub(call: types.CallbackQuery):
    if await check_subscription(call.from_user.id):
        await call.answer("✅ Спасибо за подписку!", show_alert=True)
        await call.message.delete()
        await show_menu(call.message.chat.id, str(call.from_user.id))
    else:
...
async def admin_add_channel(message: types.Message):
    ...
    try:
        ...
        if len(args) < 4:
            await message.answer("Использование: /add_channel [ID] [URL] [Название]")
            return
        cid, url, name = int(args[1]), args[2], args[3]
        await add_required_channel(cid, url, name)
...
async def admin_del_channel(message: types.Message):
    ...
    try:
        ...
        if len(args) < 2:
            await message.answer("Использование: /del_channel [ID]")
            return
        cid = int(args[1])
        await remove_required_channel(cid)
        await message.answer(f"✅ Канал {cid} удален!")
    except Exception as e:
        await message.answer(f"Ошибка: {e}")
...
async def show_menu(chat_id: int, user_id: str = None):
    ...
    buttons = [
        ...
        [types.InlineKeyboardButton(text="💰 Вывод", callback_data='withdraw'),
         types.InlineKeyboardButton(text="🎁 Ежедневный кейс", callback_data='daily')],
    ...
    if is_new_user:
        ...
        if ref_id and str(ref_id) != str(uid):
            try:
                ...
                if ref_user and ref_id_int != uid:
                    # Сохраняем связь, но не даем награду сразу
                    async with db_pool.acquire() as conn:
                        await conn.execute(
                            'INSERT INTO referral_connections (user_id, referrer_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
                            uid, ref_id_int
                        )
                    print(f"[REFERRAL] Connection saved via start: {uid} invited by {ref_id_int}. Reward pending first case open.")
            except ValueError:
                print(f"[REFERRAL] ERROR: Invalid ref_id format: {ref_id}")
...
async def support_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("✍️ <b>Напишите ваше сообщение в техподдержку:</b>", parse_mode='HTML', reply_markup=markup)
...
async def reply_to_user_callback(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        uid = str(callback.from_user.id)
        new_state = {'state': 'answering_support', 'target_user_id': user_id, 'message_to_edit': callback.message.message_id, 'chat_to_edit': callback.message.chat.id}
        ...
        await callback.message.answer(f"✍️ <b>Введите ответ для пользователя</b> <code>{user_id}</code>:", parse_mode='HTML', reply_markup=markup)
        await callback.answer()
    except Exception as e:
...
async def reply_to_admin_callback(callback: types.CallbackQuery, state: FSMContext):
    try:
        uid = str(callback.from_user.id)
        user_states[uid] = {'state': 'answering_admin', 'message_to_edit': callback.message.message_id, 'chat_to_edit': callback.message.chat.id}
        await set_user_state(callback.from_user.id, user_states[uid])

        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="❌ Отмена", callback_data='menu')]
        ])

        await callback.message.answer("✍️ <b>Введите ваш ответ поддержке:</b>", parse_mode='HTML', reply_markup=markup)
        await callback.answer()
    except Exception as e:
...
async def handle_query(call: types.CallbackQuery):
            ...
            if ref_id:
                ...
                if is_new_user:
                    ...
                    if ref_user and ref_id != user_id_int:
                        ...
                        print(f"[REFERRAL] Connection saved: {user_id} invited by {ref_id}. Reward pending first case open.")
    ...
    if data == 'menu':
        ...
        await bot.send_photo(
            ...
            caption=(
                f"⭐️ <b>Зарабатывай звезды приглашая друзей!</b> ⭐️\n\n"
                f"👋 <b>Где искать рефералов?</b>\n"
                f"🔸Приглашай в приложение своих друзей\n"
                f"🔸Оставь свою ссылку в своём канале\n"
                f"🔸Отправляй её в разные чаты\n\n"
                f"🚀 <b>Награда:</b> За каждого приглашенного ты получишь <b>2 ⭐️</b>\n\n"
                f"⚠️ <b>Важно:</b> Чтобы реферал засчитался и ты получил награду, он должен открыть свой первый <b>Ежедневный кейс</b>. Мы ценим только активных игроков! ✨\n\n"
        ...
        # Проверяем КД перед показом кнопки открытия
        user_data = await get_user(user_id_int)
        now = time.time()
        if now - user_data['last_bonus'] < 86400:
            await bot.send_photo(
                chat_id, images['bonus'],
                caption="⏱ Кейс уже открыт сегодня. Возвращайся завтра!",
                reply_markup=back_markup
            )
            return

        jackpot_amount = await get_jackpot_amount()
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🔑 Открыть кейс", callback_data='open_case')],
        ...
        await bot.send_photo(
            chat_id, images['bonus'],
            caption=(
                f"🎁 <b>Ежедневный кейс</b>\n\n"
                f"Испытай свою удачу и выиграй ценные призы!\n"
                f"💰 Текущий джекпот: <b>{jackpot_amount:.2f} ⭐️</b>\n\n"
                f"Нажми кнопку ниже, чтобы открыть кейс:"
            ),
            reply_markup=markup,
            parse_mode='HTML'
        )
        ...
        # Переносим словарь видео внутрь хендлера или делаем его глобальным
        # Для исправления синтаксиса, убедимся что словарь находится внутри или корректно снаружи
        daily_videos = {
            0.5: "attached_assets/0.5_1767819250926.MP4",
            1.0: "attached_assets/1_1767819250926.MOV",
            1.5: "attached_assets/1.5_1767819250926.MOV",
            2.0: "attached_assets/2_1767819250926.MOV",
            2.5: "attached_assets/2.5_1767819250926.MOV",
            3.0: "attached_assets/3_1767819250926.MOV",
            5.0: "attached_assets/5_1767819250926.MP4",
            10.0: "attached_assets/10_1767819250926.MOV",
            20.0: "attached_assets/20_1767819250926.MOV",
            "JACKPOT": "attached_assets/Джекпот__1767819250926.MP4"
        }

        # Каждое из видео выше должно существовать в attached_assets
        # Если видео не отправлялось, возможно проблема в типах данных (float vs int)
        # Приведем ключ к float для надежности
        video_key = "JACKPOT" if is_jackpot else float(amount)
        video_path = daily_videos.get(video_key)
        ...
        if video_path and os.path.exists(video_path):
            ...
            sent_video = await bot.send_video(
                ...
                caption="🎡 Крутим колесо фортуны..."
            )
        else:
            await bot.send_message(chat_id, "🎡 Крутим колесо фортуны...")

        # Задержка 7 секунд перед результатом
        await asyncio.sleep(7)
        
        if is_jackpot:
            msg = (
                f"🎉🎉🎉 <b>ДЖЕКПОТ!!!</b> 🎉🎉🎉\n\n"
                f"Это невероятно! Вы выиграли весь банк джекпота!\n"
                f"💰 Ваша награда: <b>{amount:.2f} ⭐️</b>\n\n"
                f"Легенда!"
            )
        else:
            msg = (
                f"🎁 <b>Ежедневный кейс открыт!</b>\n\n"
                f"Ваш выигрыш составил: <b>{amount} ⭐️</b>\n"
                f"Удача сегодня на вашей стороне! ✨\n\n"
                f"Возвращайтесь завтра за новой порцией удачи!"
            )
        ...
        if not trophies:
            ...
            text = (
                f"🏅 <b>МОИ НАГРАДЫ</b>\n\n"
                f"🏆 Кубок получен за победу в событии «{trophy['tournament_name']}»!\n\n"
...
async def handle_user_input(message: types.Message):
    ...
    if state == 'awaiting_promo':
        ...
        # Если промокод неверный или исчерпан, оставляем состояние ожидания
        if result['success']:
            user_states[uid] = None
            await set_user_state(uid_int, None)
        return
        ...
        try:
            ...
            if message.sticker:
                await bot.send_message(target_uid, admin_info, parse_mode='HTML')
                await bot.send_sticker(target_uid, message.sticker.file_id, reply_markup=markup)
            elif message.photo:
                await bot.send_photo(
                    target_uid, 
                    message.photo[-1].file_id, 
                    caption=f"{admin_info}\n\n{message.caption or ''}",
                    parse_mode='HTML', 
                    reply_markup=markup
                )
                ...
                await bot.send_message(target_uid, admin_info, parse_mode='HTML')
                await bot.send_voice(target_uid, message.voice.file_id, reply_markup=markup)
            elif message.video_note:
                await bot.send_message(target_uid, admin_info, parse_mode='HTML')
                await bot.send_video_note(target_uid, message.video_note.file_id, reply_markup=markup)
            ...
            await message.reply(f"✅ <b>Ответ успешно отправлен пользователю</b> <code>{target_uid}</code>", parse_mode='HTML')

            # Помечаем исходное сообщение как отвеченное
            msg_to_edit = state_raw.get('message_to_edit')
            chat_to_edit = state_raw.get('chat_to_edit')
            if msg_to_edit and chat_to_edit:
                try:
                    await bot.edit_message_reply_markup(
                        chat_id=chat_to_edit,
                        message_id=msg_to_edit,
                        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                            [types.InlineKeyboardButton(text="✅ Отвечено", callback_data='noop')]
                        ])
                    )
                except:
                    pass
    ...
    elif state == 'answering_admin' or state == 'awaiting_support_reply':
        uid_int = message.from_user.id
        username = message.from_user.username or "нет"
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="✏️ Ответить", callback_data=f"reply_to_user:{uid_int}")]
        ])
        
        prefix = "🆘 <b>Ответ пользователя!</b>" if state == 'answering_admin' else "🆘 <b>Новое сообщение в поддержку!</b>"
        admin_info = f"{prefix}\n👤 <b>Пользователь:</b> @{username}\n🆔 <b>ID:</b> <code>{uid_int}</code>"
        ...
        try:
            if message.sticker:
                ...
                await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
            elif message.animation:
                await bot.send_animation(ADMIN_ID, message.animation.file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
            elif message.video:
                await bot.send_video(ADMIN_ID, message.video.file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
            elif message.voice:
                await bot.send_voice(ADMIN_ID, message.voice.file_id, caption=admin_info, parse_mode='HTML', reply_markup=markup)
            elif message.video_note:
                await bot.send_message(ADMIN_ID, admin_info, parse_mode='HTML')
                await bot.send_video_note(ADMIN_ID, message.video_note.file_id, reply_markup=markup)
            elif message.document:
                await bot.send_document(ADMIN_ID, message.document.file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
            else:
                await bot.send_message(ADMIN_ID, f"{admin_info}\n\n{message.text or ''}", parse_mode='HTML', reply_markup=markup)
            
            await message.answer("✅ <b>Сообщение отправлено!</b>", parse_mode='HTML')

            # Помечаем исходное сообщение как отвеченное (если это был ответ на сообщение)
            msg_to_edit = state_raw.get('message_to_edit')
            chat_to_edit = state_raw.get('chat_to_edit')
            if msg_to_edit and chat_to_edit:
                try:
        ...
        if target_user_id:
            markup = types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="✏️ Ответить", callback_data=f"reply_to_admin:{ADMIN_ID}")]
            ])
            admin_info = "📩 <b>Сообщение от поддержки:</b>"
            try:
                if message.sticker:
                    ...
                    await bot.send_video(target_user_id, message.video.file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
                elif message.voice:
                    await bot.send_voice(target_user_id, message.voice.file_id, caption=admin_info, parse_mode='HTML', reply_markup=markup)
                elif message.video_note:
                    await bot.send_message(target_user_id, admin_info, parse_mode='HTML')
                    await bot.send_video_note(target_user_id, message.video_note.file_id, reply_markup=markup)
                elif message.document:
                    await bot.send_document(target_user_id, message.document.file_id, caption=f"{admin_info}\n\n{message.caption or ''}", parse_mode='HTML', reply_markup=markup)
                else:
        ...
        admin_info = f"🆘 <b>Новое сообщение в техподдержку!</b>\n👤 <b>Пользователь:</b> @{username}\n🆔 <b>ID:</b> <code>{uid_int}</code>"
        ...
        try:
            ...
            await message.answer("✅ <b>Ваше сообщение отправлено в поддержку!</b>", parse_mode='HTML')
        except Exception as e:
            await message.answer(f"❌ Ошибка при отправке: {e}")
        ...
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="✏️ Ответить", callback_data=f"support")]
        ])
        ...
        try:
            ...
            if message.sticker:
                ...
                await bot.send_message(target_user_id, admin_info, parse_mode='HTML')
                await bot.send_voice(target_user_id, message.voice.file_id, reply_markup=markup)
            elif message.video_note:
                await bot.send_message(target_user_id, admin_info, parse_mode='HTML')
                await bot.send_video_note(target_user_id, message.video_note.file_id, reply_markup=markup)
            elif message.document:
            ...
            await message.reply(f"✅ <b>Ответ успешно отправлен пользователю</b> <code>{target_user_id}</code>", parse_mode='HTML')
...
async def set_bot_commands():
    commands = [
        ...
        types.BotCommand(command="daily", description="🎁 Ежедневный кейс"),
